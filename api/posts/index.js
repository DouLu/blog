...
async function posts_default(req, res) {
  if (supabaseUrl && supabaseKey) {
    ...
    switch (req.method) {
      case "GET":
        try {
          const userId2 = req?.cookies?.token;
          if (userId2) {
            const { data: data2 } = await supabase.from("post").select().eq("author_id", userId2);
            res.status(200).json({ success: true, data: data2 });
          } else {
            const { data: data2 } = await supabase.from("post").select();
            res.status(200).json({ success: true, data: data2 });
          }
        } catch (error2) {
      ...
      case "POST":
        ...
        if (data) {
          ...
          try {
            const { data: data2, error: error2 } = await supabase.from("post").insert(rowData);
            if (error2) {
              res.status(200).json({ success: false, message: error2 });
            } else {
              res.status(200).json({ success: true, message: "\u6587\u7AE0\u53D1\u8868\u6210\u529F\uFF01" });
            }
          } catch (error2) {
